/* ============================================================================
 * Atlaz · TradeLoopLedgerService
 * Records every key event of a SKU's journey from opportunity to reorder.
 * In-memory for the MVP; structured to persist to Room/SQLDelight in production.
 * Global: TradeLoopLedgerService
 * ==========================================================================*/
const TradeLoopLedgerService = (() => {
  // Seed with mock events from DB, then append runtime events.
  const events = (typeof DB !== "undefined" ? DB.ledgerEvents.slice() : []);
  let counter = 9000;
  const listeners = [];
  ...
  function record(skuPassportId, eventType, opts = {}) {
    const actorType = opts.actorType ||
      (eventType.startsWith("human") ? "user"
        : /generated|analyzed|matched|calculated|checked|discovered/.test(eventType) ? "ai"
        : eventType.includes("supplier") ? "supplier" : "system");
    const ev = {
      eventId: `LE-${counter++}`,
      skuPassportId,
      eventType,
      ...
      confidenceScore: opts.confidenceScore != null ? opts.confidenceScore : (actorType === "ai" ? 0.78 : 1),
